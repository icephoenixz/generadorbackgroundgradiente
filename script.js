...
// Agregar cadena "deg" al ángulo actual
...
// devolver el código rgb del selector de color del código hexadecimal
const getRgbCode = () => {
...
/*
*  ----------------------------------  Cambiar ángulo con el círculo  -----------------------------------
* */


// Si alguna cuerda = 0 => ya sabemos el ángulo
const checkCoordIsZero = (x, y) => {

  // Compruebe si el ángulo es cero / 90 ° / 180 ° o 270 °
...
// Obtenga las coordenadas x e y del clic
const getCursorPosition = e => {

  // Obtener posición relacionada con la ventana gráfica
  let circle = e.target.getBoundingClientRect();
  // Agregar desplazamiento arriba y a la izquierda
  let scrollLeft = window.pageXOffset;
  let scrollTop = window.pageYOffset;
  // Obtenga coordenadas X e Y de clic
  let circleXaxis = e.pageX;
  let circleYaxis = e.pageY;
  // Obtenga coordenadas de clic relacionadas con el centro del círculo
  // Queremos un eje X e Y regular, entonces =>
  // x valores pos = derecha | x valores neg = izquierda
  let x = circleXaxis - (circle.left + scrollLeft + circle.width / 2);
  // y valores pos = top | y neg valores = fondo
...
const getCircleQuarter = (x, y) => {
  if(Math.sign(x) === 1 && Math.sign(y) === 1){
    // Primer trimestre (0 ° a 90 °)
    return {x, y, quarter: 1}
  } else if(Math.sign(x) === 1 && Math.sign(y) === -1){
    // Segundo trimestre (90 ° a 180 °)
    return {x, y, quarter: 2}
  } else if(Math.sign(x) === -1 && Math.sign(y) === -1) {
    // Tercer trimestre (180 ° a 270 °)
    return {x, y, quarter: 3}
  } else if(Math.sign(x) === -1 && Math.sign(y) === 1) {
    // Cuarto trimestre (270 ° a 0 °)
    return {x, y, quarter: 4}
  } else {
    console.log('error: los parámetros de la función no son coordenadas válidas')
  }
};
...
const getNewAngle = (e) => {
  ...
  // Caso donde no 0 en coordenadas => devuelve un objeto con las coordenadas
  if(isNaN(cursorPosition)) {
    ...
    // Caso donde había un 0 en las coordenadas => devolver el ángulo
...
// Cambiar la posición del punto en el círculo correspondiente al ángulo
...
// Obtiene todos los nodos de texto en cada elemento secundario del primer parámetro
...
// Consigue el código del editor de texto
const getString = () => {
